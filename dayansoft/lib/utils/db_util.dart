import 'package:dayansoft/db/sql_manager.dart';
...
class DBUtil {
  ...
  static Future<List<Map>> queryListByHelper(String tableName,List<String> selects,String whereStr,List whereArgs) async {
  ...
  ///sql原生查找列表
  static Future<List<Map>> queryList(String sql) async {
    return await db.rawQuery(sql);
  }

  ///sql助手修改
  static Future<int> updateByHelper(String tableName,Map<String,Object> setArgs,String whereStr,List whereArgs) async {
    //样例：
  ...
  static Future<int> deleteByHelper(String tableName,String whereStr,List whereArgs) async {
  ...
  ///事务控制
  static Future<dynamic> transaction(Future<dynamic> Function(Transaction txn) action)async{
    //调用样例
    //  try {
  ...
  open() async {
    ...
    String path = join(databasePath,SqlManager.DB_NAME);

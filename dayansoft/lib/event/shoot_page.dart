...
//              },
  ...
  void onNewCameraSelected(CameraDescription cameraDescription) async {
    ...
    controller.addListener(() {
      ...
      if (controller.value.hasError) {
        ToastUtil.show('Camera error ${controller.value.errorDescription}');
      }
    });
  ...
  void onTakePictureButtonPressed() {
    takePicture().then((String filePath) {
      if (mounted) {
        ...
        if (filePath != null) ToastUtil.show('图片保存到$filePath');
      }
    });
  }
  ...
  void onVideoRecordButtonPressed() {
    startVideoRecording().then((String filePath) {
      if (mounted) setState(() {});
      if (filePath != null) ToastUtil.show('开始录制');
    });
  }
  ...
  void onStopButtonPressed() {
    stopVideoRecording().then((_) {
      if (mounted) setState(() {});
      ToastUtil.show('视频记录到$videoPath');
    });
  }
  ...
  void onResumeButtonPressed() {
    resumeVideoRecording().then((_) {
      if (mounted) setState(() {});
      ToastUtil.show('录制视频恢复');
    });
  }

  Future<String> startVideoRecording() async {
    if (!controller.value.isInitialized) {
      ToastUtil.show('异常: 首先选择一个相机');
      return null;
    }
